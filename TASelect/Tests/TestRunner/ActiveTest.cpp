...
void ActiveTest::setTestResult( CPPUNIT_NS::TestResult *result )
{ 
...
void ActiveTest::run( CPPUNIT_NS::TestResult *result )
{
	CWinThread *thread;
	...
	::DuplicateHandle( GetCurrentProcess(), thread->m_hThread, GetCurrentProcess(), &m_threadHandle, 0, FALSE, DUPLICATE_SAME_ACCESS );
...
{
  ActiveTest *test = static_cast<ActiveTest *>(thisInstance);
