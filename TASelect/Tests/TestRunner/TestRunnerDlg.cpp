...
TestRunnerDlg::TestRunnerDlg( TestRunnerModel *model, int nDialogResourceId, CWnd *pParent )
    : cdxCDynamicDialog( nDialogResourceId, pParent )
{
	init( model );
}

TestRunnerDlg::TestRunnerDlg( TestRunnerModel *model, const TCHAR *szDialogResourceId, CWnd *pParent )
...
/*
// TODO : Check if ok to remove (Not sure why we need an accelerator)
...
// It always fails!!! I don't understand why. Complain about not finding the resource name!
  ...
  */
	...
	int total_col_1_4 = m_settings.col_1 + m_settings.col_2 + m_settings.col_3 + m_settings.col_4;
	...
	formatter.AddColumn( "Type", m_settings.col_1, LVCFMT_LEFT, 0 );
	formatter.AddColumn( "Name", m_settings.col_2, LVCFMT_LEFT, 1 );
	formatter.AddColumn( "Failed Condition", m_settings.col_3, LVCFMT_LEFT, 2 );
	m_listCtrl.setLineNumberSubItem( formatter.GetNextColumnIndex() );
	formatter.AddColumn( "Line Number", m_settings.col_4, LVCFMT_LEFT, 3 );
	m_listCtrl.setFileNameSubItem( formatter.GetNextColumnIndex() );
	formatter.AddColumn( "File Name", col_5_width, LVCFMT_LEFT, 4 );
...
void TestRunnerDlg::addListEntry( const CPPUNIT_NS::TestFailure &failure )
{
	CListCtrl *listCtrl = (CListCtrl *)GetDlgItem ( IDC_LIST );

	int currentEntry = m_result->testErrors() + m_result->testFailures() -1;
	...
	// Set test name
	setter.addSubItem( CString(failure.failedTestName().c_str()), errorType );
	...
	// Set the file name
	setter.addSubItem( CString(failure.sourceLine().fileName().c_str()) );
...
void TestRunnerDlg::startTest( CPPUNIT_NS::Test *test )
{
	CWnd *runningTestCaseLabel = GetDlgItem(IDC_RUNNING_TEST_CASE_LABEL);
  
	if( NULL != runningTestCaseLabel )
	{
		runningTestCaseLabel->SetWindowText( CString( test->getName().c_str() ) );
	}
}

void TestRunnerDlg::addFailure( const CPPUNIT_NS::TestFailure &failure )
{
...
void TestRunnerDlg::endTest( CPPUNIT_NS::Test *test )
{
	if( 0 == m_selectedTest )
	{
		return;
...
void TestRunnerDlg::updateCountsDisplay()
{
	...
	argumentString.Format( _T("Execution time: %3.3lf seconds"), ( m_testEndTime - m_testStartTime ) / 1000.0 );
...
void TestRunnerDlg::OnSelectTestInHistoryCombo() 
{
	// Can return CB_ERR (-1)
	signed int currentSelection = getHistoryCombo()->GetCurSel();

	if( currentSelection >= 0 && currentSelection < (signed int)m_model->history().size() )
	{
...
void TestRunnerDlg::updateHistoryCombo()
{
	...
	for( TestRunnerModel::History::const_iterator it = history.begin(); it != history.end(); ++it )
	{
...
void TestRunnerDlg::OnBrowseTest() 
{
	TreeHierarchyDlg dlg( this);
...
BOOL TestRunnerDlg::PreTranslateMessage( MSG *pMsg )
{
...
CComboBox *TestRunnerDlg::getHistoryCombo()
{
	CComboBox *comboBox = (CComboBox *)GetDlgItem( IDC_COMBO_TEST );
	ASSERT (comboBox);
...
void TestRunnerDlg::loadSettings()
{
...
void TestRunnerDlg::saveSettings()
{
	m_settings.autorunOnLaunch = ( m_bAutorunAtStartup != 0 );
	StoreWindowPosition( TestRunnerModel::settingKey, TestRunnerModel::settingMainDialogKey );
...
TestRunnerModel &TestRunnerDlg::model()
{
	ASSERT( m_model != NULL );
	return *m_model;
}

void TestRunnerDlg::OnClose() 
{
	OnOK();
}

CRect TestRunnerDlg::getItemWindowRect( unsigned int itemId )
{
...
CRect TestRunnerDlg::getItemClientRect( unsigned int itemId )
{
...
void TestRunnerDlg::initializeLayout()
{
...
void TestRunnerDlg::OnSize( UINT nType, int cx, int cy ) 
{
	cdxCDynamicDialog::OnSize(nType, cx, cy);
	updateListColumnSize();
}

void TestRunnerDlg::updateListColumnSize()
{
...
void TestRunnerDlg::OnSelectedFailureChange( NMHDR *pNMHDR, LRESULT *pResult )
{
...
void TestRunnerDlg::displayFailureDetailsFor( unsigned int failureIndex )
{
	CString details;
	...
	int iCurPos = 0;
	CString strValue = details.Tokenize( _T("\r\n"), iCurPos );
	...
	do
	{
		m_details.SetCurSel( m_details.InsertString( m_details.GetCount(), strValue ) );

		CDC *pDC = m_details.GetDC();
		CSize sz = pDC->GetTextExtent( strValue );

		if( sz.cx > m_iLargestText )
		{
			m_details.SetHorizontalExtent( sz.cx );
			m_iLargestText = sz.cx;
		}

		m_details.ReleaseDC( pDC );

		strValue = details.Tokenize( _T("\r\n"), iCurPos );
	}while( false == strValue.IsEmpty() );
}

void TestRunnerDlg::initializeFixedSizeFont()
{
	LOGFONT font;
	GetFont()->GetLogFont( &font );
	font.lfPitchAndFamily = FIXED_PITCH | ( font.lfPitchAndFamily & ~15 );
	m_fixedSizeFont.CreateFontIndirect( &font );
}

int TestRunnerDlg::CheckFile()
{
	CFileTxt inf;
