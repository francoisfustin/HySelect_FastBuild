...
CDlgWizardPM_RightViewInput_SolarTemperature::CDlgWizardPM_RightViewInput_SolarTemperature( CWizardSelPMParams *pclWizardSelPMParams, CWnd *pclParent )
	: CDlgWizardPM_RightViewInput_Base( pclWizardSelPMParams, SolarTemperature, IDD, pclParent )
{
	m_bGoToAdvModeMsgSent = false;
}

void CDlgWizardPM_RightViewInput_SolarTemperature::ApplyPMInputUserUpdated( bool bWaterCharUpdated, bool bShowErrorMsg )
{
...
int CDlgWizardPM_RightViewInput_SolarTemperature::IsAtLeastOneError( int &iErrorMaskNormal, int &iErrorMaskAdvanced )
{
...
BEGIN_MESSAGE_MAP( CDlgWizardPM_RightViewInput_SolarTemperature, CDlgWizardPM_RightViewInput_Base )
	ON_WM_LBUTTONDOWN()
	ON_CBN_SELCHANGE( IDC_COMBO_NORM, OnCbnSelChangeNorm )
	ON_EN_SETFOCUS( IDC_EDIT_SFTYTEMP, OnEnSetFocusSafetyTempLimiter )
	ON_EN_CHANGE( IDC_EDIT_SFTYTEMP, OnEnChangeEditSafetyTempLimiter )
END_MESSAGE_MAP()

void CDlgWizardPM_RightViewInput_SolarTemperature::DoDataExchange( CDataExchange *pDX )
{
...
BOOL CDlgWizardPM_RightViewInput_SolarTemperature::OnInitDialog()
{
	...
	str = TASApp.LoadLocalizedString( IDS_DLGWIZPM_RVIEWINPUTSOLARTEMP_NORM );
...
void CDlgWizardPM_RightViewInput_SolarTemperature::OnPaint()
{
...
void CDlgWizardPM_RightViewInput_SolarTemperature::OnLButtonDown( UINT nFlags, CPoint point )
{
	if( false == m_bGoToAdvModeMsgSent )
	{
		...
		if( TRUE == rectGoButton.PtInRect( (POINT)point ) )
		{
			...
			m_pclParent->PostMessage( WM_USER_RIGHTVIEWWIZPM_SWITCHMODE, WPARAM( SolarTemperatureAdvance ), 0 );
		}
	}
...
void CDlgWizardPM_RightViewInput_SolarTemperature::OnCbnSelChangeNorm()
{
...
void CDlgWizardPM_RightViewInput_SolarTemperature::OnEnSetFocusSafetyTempLimiter()
{
	m_ExtEditSafetyTempLimiter.SetSel( 0, -1 );
	m_dSafetyTempLimiterSaved = m_ExtEditSafetyTempLimiter.GetCurrentValSI();
}

void CDlgWizardPM_RightViewInput_SolarTemperature::OnEnChangeEditSafetyTempLimiter()
{
	if( NULL == m_pclWizardSelPMParams || NULL == m_pclWizardSelPMParams->m_pclSelectPMList || NULL == m_pclWizardSelPMParams->m_pclSelectPMList->GetpclPMInputUser() )
	{
		ASSERT_RETURN;
	...
	// Check if errors.
	int iErrorCode = IsHeatingSolarSafetyTempLimiterOK( &m_ExtEditSafetyTempLimiter );
	...
	if( 0 == iErrorCode )
	{
		...
		( (CRViewWizardSelPM *)m_pclParent )->OnInputChange( iErrorCode, Error_Temp_SafetyTempLimiter, 
				Error_Temp_SafetyTempLimiter | Error_Temp_Supply );
	}
}

LRESULT CDlgWizardPM_RightViewInput_SolarTemperature::OnUnitChange( WPARAM wParam, LPARAM lParam )
{
...
void CDlgWizardPM_RightViewInput_SolarTemperature::GetExtEditList( CArray<CExtNumEdit*> &arExtEditList )
{
	arExtEditList.Add( &m_ExtEditSafetyTempLimiter );
}

void CDlgWizardPM_RightViewInput_SolarTemperature::_VerifyInputValues()
{
	IsHeatingSolarSafetyTempLimiterOK( &m_ExtEditSafetyTempLimiter );
}
