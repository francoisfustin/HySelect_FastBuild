...
#define JSON_ASSERT_MESSAGE( condition, message ) if( !( condition ) ){ std::string str = std::wstring_convert<std::codecvt<TCHAR, char, mbstate_t>>().to_bytes( message ); throw std::runtime_error( str.c_str() ); };
...
public:
   ...
   virtual TCHAR *duplicateStringValue( const TCHAR *value, unsigned int length = unknown )
   {
      ...
      TCHAR *newString = static_cast<TCHAR *>( malloc( ( length + 1 ) * 2 ) );
      memcpy( newString, value, length * 2 );
      newString[length] = 0;
      return newString;
   }
...
void Value::CommentInfo::setComment( const TCHAR *text )
{
   if( comment_ )
   {
      valueAllocator()->releaseStringValue( comment_ );
   ...
   JSON_ASSERT( text );
   JSON_ASSERT_MESSAGE( text[0] == _T('\0') || text[0] == _T('/'), _T("Comments must start with /") );
...
const TCHAR * Value::CZString::c_str() const
{
   return cstr_;
}
...
Value::Value( const TCHAR *beginValue, 
              const TCHAR *endValue )
   : type_( stringValue )
   , allocated_( true )
   , comments_( 0 )
# ifdef JSON_VALUE_USE_INTERNAL_MAP
...
Value::Value( const _string &value )
   : type_( stringValue )
   , allocated_( true )
   , comments_( 0 )
# ifdef JSON_VALUE_USE_INTERNAL_MAP
...
const TCHAR *Value::asCString() const
{
   JSON_ASSERT( type_ == stringValue );
   return value_.string_;
}

_string Value::asString() const
{
   switch ( type_ )
   {
   case nullValue:
...
Value &Value::resolveReference( const TCHAR *key, bool isStatic )
{
...
Value &Value::operator[]( const _string &key )
{
   return (*this)[ key.c_str() ];
}

const Value &Value::operator[]( const _string &key ) const
{
   return (*this)[ key.c_str() ];
}
...
Value Value::get( const TCHAR *key, const Value &defaultValue ) const
{
   const Value *value = &((*this)[key]);
   return value == &null ? defaultValue : *value;
}

Value Value::get( const _string &key, const Value &defaultValue ) const
{
   return get( key.c_str(), defaultValue );
}

Value Value::removeMember( const TCHAR* key )
{
   JSON_ASSERT( type_ == nullValue  ||  type_ == objectValue );
...
Value Value::removeMember( const _string &key )
{
...
bool Value::isMember( const TCHAR *key ) const
{
   const Value *value = &((*this)[key]);
   return value != &null;
}

bool Value::isMember( const _string &key ) const
{
...
Value::Members Value::getMemberNames() const
{
   ...
   for ( ; it != itEnd; ++it )
   {
      members.push_back( _string( (*it).first.c_str() ) );
   }
...
void Value::setComment( const TCHAR *comment, CommentPlacement placement )
{
   if ( !comments_ )
   {
      comments_ = new CommentInfo[numberOfCommentPlacement];
...
void Value::setComment( const _string &comment, CommentPlacement placement )
{
...
_string Value::getComment( CommentPlacement placement ) const
{
...
_string Value::toStyledString() const
{
   StyledWriter writer;
...
PathArgument::PathArgument( const _string &key )
   : key_( key.c_str() )
   , kind_( kindKey )
{
}
...
void Path::makePath( const _string &path, const InArgs &in )
{
   ...
   while ( current != end )
   {
      if ( *current == _T('[') )
      {
         ...
         else
         {
            ...
            for ( ; current != end && *current >= _T('0')  &&  *current <= _T('9'); ++current )
            {
               index = index * 10 + Value::UInt(*current - _T('0') );
            }

            args_.push_back( index );
         }

         if ( current == end  ||  *current++ != _T(']') )
         {
      ...
      else
      {
         ...
         while ( current != end  &&  !_tcschr( _T("[."), *current ) )
            ++current;
         args_.push_back( _string( beginName, current ) );
      }
   }
}

void Path::addPathInArg( const _string &path, const InArgs &in, InArgs::const_iterator &itInArg, PathArgument::Kind kind )
{
   if ( itInArg == in.end() )
   {
...
void Path::invalidPath( const _string &path, int location )
{
   // Error: invalid path.
}
