...
#include <stdlib.h> 
#include <string.h> 
#include "BlowFish.h" // class's header file
...
int CBlowFish::init(const unsigned char *key, int len)
{
...
inline unsigned long CBlowFish::F(b32 *x) {
...
void CBlowFish::wencrypt(b32 *w1,b32 *w2)
{
...
void CBlowFish::wdecrypt(b32 *w1,b32 *w2)
{
...
void CBlowFish::encrypt(unsigned char *b64) {
...
void CBlowFish::decrypt(unsigned char *b64) {
// w1&2 are here big endian, convert
  b32 *w1 = (b32*)b64;
  b32 *w2 = (b32*)(b64+4);
  w1->word = bigendian2system_long(w1->word);
...
inline unsigned long CBlowFish::system2bigendian_long(unsigned long l) {
  b32 u,r;
  u.word = l;
...
inline unsigned long CBlowFish::bigendian2system_long(unsigned long l) {
  b32 u,r;
  u.word = l;
...
int CBlowFish::sencrypt(unsigned char *data, int length, const char * mode,const unsigned char * b0) {
...
int CBlowFish::sdecrypt(unsigned char *data, int length, const char * mode,const unsigned char * b0) {
  if (data == NULL) return -4;
  if (length%8!=0) return -1;
  if (mode == NULL) return -2;
...
// no copy - private
CBlowFish& CBlowFish::operator=(const CBlowFish& r) { return *this; }
CBlowFish::CBlowFish(const CBlowFish& r) { return; }