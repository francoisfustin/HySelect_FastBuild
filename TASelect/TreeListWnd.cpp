...
// IMI Hydronic Engineering functions that can be called with 'TVM_TAHYDRONICSMETHODS'.
static int TAEnsureVisible( TreeListData* pData, TAEnsureVisibleParam* pParam );
static int TAGetItemRect( TreeListData* pData, TAGetItemRectParam* pParam );

// IMI Hydronic Engineering private functions.
static void _TAComputeWiderItem( TreeListData* pData );
static void _TAGetItemCoord( TreeListData* pData, unsigned uItem, CPoint& PtCoord, bool fTextOnly );
...
//*
//*****************************************************************************
//	It's the window function for the Edit window
static LRESULT CALLBACK EditProc( HWND hWnd, UINT uMsg, WPARAM wParam, LPARAM lParam )
{
		...
		if( ( pData->uStyleEx&TVS_EX_STEPOUT ) && !( GetAsyncKeyState( VK_SHIFT ) & 0x8000 ) )
		{
			switch( wParam )										// Aus Fenster springen
			{
				...
				case VK_RIGHT:	if( pData->uEditMode && iId == 3 )break;
					...
					SendMessage( hWnd, EM_GETSEL, (WPARAM)&dwStart, (LPARAM)&dwStop );
					dwCount = SendMessage( hWnd, EM_LINELENGTH, 0, 0 );
	...
	{
		iRet = CallWindowProc( pProc, hWnd, uMsg, wParam, lParam );
...
//*
//*****************************************************************************
//	It's the window function for the tool tip window
...
//	pData		: Zeiger auf die Fensterdaten
static void CreatToolTip( TreeListData *pData )
{
	TOOLINFO	sInfo;
...
//*
//*****************************************************************************
//	Creates an image list with two check boxes
//	pData		: Pointer to the data window
//	iMode		: Which image list will be generated (0=hStates 1=hChecks)
...
static HIMAGELIST CreateDragImage( TreeListData *pData, unsigned uItem, unsigned uSub )
{
	...
	{													// Image für Haupteintrag erzeugen
		pText = pEntry->pText;
...
static void UpdateColors( TreeListData *pData )
{
	...
	if( !pData->cColorChanged[TVC_COLBK] )pData->uColors[TVC_COLBK] = GetSysColor( COLOR_WINDOW );
	if( !pData->cColorChanged[TVC_COLODD] )pData->uColors[TVC_COLODD] = GetSysColor( COLOR_BTNSHADOW );
	if( !pData->cColorChanged[TVC_COLEVEN] )pData->uColors[TVC_COLEVEN] = GetSysColor( COLOR_WINDOW );
...
static int UpdateHeight( TreeListData *pData )
{
	...
	if( iHeight < pData->iSubImgYsize + 2 )iHeight = pData->iSubImgYsize + 2;
...
static int UpdateRect( TreeListData *pData, unsigned uItem, unsigned uSub )
{
	...
	sRect.right = pData->aColumnXpos[uSub + 1];
...
static int UpdateColRect( TreeListData *pData, unsigned uColumn )
{
	...
	sRect.right = pData->aColumnXpos[uColumn + 1];
	sRect.right -= pData->uScrollX;
...
static void UpdateScrollX( TreeListData *pData )
{
	...
	// TAHydronics
	if( true == pData->fVAlternateMode )
	{
		_TAComputeWiderItem( pData );
		uSize = pData->uTAWiderItem;
	}
...
static int UpdateColumns( TreeListData *pData )
{
	...
	for( uPos = 0; uPos < pData->uColumnCount; )					// Suche die erste geänderte Spalte
	{
		iSize = pData->aColumn[uPos].sReal;
		iOld = iNow;
		iNow += iSize;
		uPos += 1;
		...
		pData->aColumnXpos[uPos] = iNow;
		break;
	}


	while( uPos < pData->uColumnCount )							// Restliche Spalten berechen
	{
		iOld = iNow;
		iNow += pData->aColumn[uPos].sReal;
		uPos += 1;
...
//	Zeichnet einen Eintrag neu
//	pData		: Pointer to the data window
//	uItem		: Is the number of entry
//	uFlags		: Bit 0   : 0=Full line  1=text only
//				  Bit 7   : 1=Only column
//				  Bit 8.. : column number
//	Returns 1 if the entry was visible
static int TreeListGetItemRect( TreeListData *pData, unsigned uItem, unsigned uFlags, RECT *pRect )
{
	ExtraItem  *pExtra;
	BaseItem   *pEntry;
	...
	pEntry = pData->pTreeItems[uItem];
	if( !pEntry->uShowPos )									// If the entry is opened
	{
		memset( pRect, 0, sizeof( RECT ) );
		return 0;
	}
	...
	uPos = pEntry->uShowPos - pData->uScrollY - 1;
	if( uPos >= pData->uMaxEnties )								// Entry in the window visible
	{
		memset( pRect, 0, sizeof( RECT ) );
		return 0;
	}
	...
	if( ( uFlags & 0xFC ) == TVIR_GETCOLUMN )						// Nur Spalten
	{
		...
		pRect->right = pData->aColumnXpos[uSub + 1];
...
//*
//*****************************************************************************
//	Makes an entry visible
//	pData		: Pointer to the data window
//	uItem		: It's the number of entry
//	uSub		: Under the entry should be visible
//						0xFFFFFFFF only line
//						FIRST_LINE as the top entry
//	Returns 1 if the item was scrolled only, or 0 if it has been opened.
...
static int TreeListDeleteItem( TreeListData *pData, unsigned uItem, int iMode )
{
	...
	if( uItem == pData->uSelectedItem )							// Einen ausgewählten Eintrag löschen
	{
		...
		UNLOCK( pData );
		SendNotify( pData, &sNotify.hdr );						// Bekanndgeben das der Eintrag nicht mehr ausgewählt ist
		LOCK( pData );
...
static int TreeListSelectItem( TreeListData *pData, unsigned uItem, unsigned uSubItem, int iMode )
{
	...
	if( pData->uStyleEx&TVS_EX_MULTISELECT )					// Ist die Mehrfachauswahl möglich
	{
		...
		{												// Alle gewählten Einträge abwählen
...
static unsigned TreeListInsertItem( TreeListData *pData, TV_INSERTSTRUCT *pInsert )
{
	...
	switch( uAfter )
	{
		...
		case U( TVI_SORTEX ):										// Alphapetisch einfügen mittels Funktion
...
static int TreeListSetItem( TreeListData *pData, const TV_ITEM *pItem )
{
	...
	// TA: It's to have possibility to disable a checkbox.
	if( uBits & TVIF_STATEEX )
	{
		const TVITEMEX *pItemEx = (const TVITEMEX *)pItem;
		pEntry->uStateEx = pItemEx->uStateEx;
	}
...
static int TreeListInsertColumn( TreeListData *pData, unsigned uCol, TV_COLUMN *pColumn )
{
	...
	if( !pData->hHeader )										// Einen neuen Header erzeugen
	{
		...
		pData->hHeader = CreateWindow( WC_HEADER, NULL, WS_VISIBLE | WS_CHILD | HDS_HORZ | HDS_BUTTONS, sRect.left, sRect.top, sRect.right, iYoff, pData->hWnd, (HMENU)1, NULL, NULL );
	...
	pData->aColumn[uCol].bFixed = bFixed;
...
static unsigned TreeListHitTest( TreeListData *pData, TV_HITTESTINFO *pInfo )
{
	...
	// if(iXpos>=pData->aColumnXpos[1])						// Auf Extraeintrag
	if( (unsigned)iXpos >= pData->aColumnXpos[1] + pData->uScrollX )
	{
		for( uSub = 1; uSub < pData->uColumnCount; uSub++ )
		{
			if( iXpos >= pData->aColumnXpos[uSub + 1] )
				continue;
			...
			switch( pData->aColumn[uSub].bAlign )				// Textausrichtung
			{
				...
				case DT_RIGHT:
					iWidth = pData->aColumnXpos[uSub + 1];
...
//*
//*****************************************************************************
//	Sets the underlined item.
//	pData	: Pointer to the data window.
//	uItem	: Is the number of entry.
//	uSub	: Is the column.
//	Returns 1 if the entry is underlined.
...
static unsigned TreeListGetNextItem( TreeListData *pData, unsigned uItem, unsigned uFlags )
{
	...
	switch( uFlags )
	{
		...
		case TVGN_NEXTITEM:			if( uItem > pData->uTreeItemsMax )
		{
			if( uItem != U( TVI_ROOT ) )return 0;
...
static unsigned TreeListFindItem( TreeListData *pData, unsigned uItem, TVFIND *pFind )
{
	...
	for( ; uItem; uItem = pEntry->uNextItem )						// Durchlaufe alle Kinder
	{
		...
		{
			if( uSub >= pData->uColumnCount )continue;
		...
		if( uTextLen != uTextSize )continue;
...
static void TreeListChangeCheckbox( TreeListData *pData, UINT uItem, int iPosX = 0, int iPosY = 0 )
{
	...
	if( pData->uStyleEx&TVS_EX_SINGLECHECKBOX )				// Einzelauswahl
	{
		...
		else if( pData->uSingleSel && pTemp )					// Anderer Eintrag gewählt
		{
	...
	if( uItem )												// Wurde auf einen Eintrag getrückt
	{
		...
		else if( sInfo.flags&uMaskItem )						// Eintrag auswählen
		{
			...
			{											// Auto-Edit mit Doppelklick
				...
				else if( TreeListStartNotifyEdit( pData, pData->uSelectedItem, 0, VK_DBLCLK, lParam ) )
				{
		...
		else if( sInfo.flags&uMaskSub )						// Extra-Eintrag auswählen
		{
			...
			if( uMsg == WM_LBUTTONDBLCLK )
			{
				...
				else if( TreeListStartNotifyEdit( pData, uItem, uSub, VK_DBLCLK, lParam ) )
				{
...
static void TreeListKeyDown( TreeListData *pData, WPARAM wParam, LPARAM lParam )
{
	...
	{													// Einen anderen Eintrag auswählen
		iSub = pData->uSelectedSub;
...
static HWND TreeListEditLabel( TreeListData *pData, unsigned uItem, unsigned uSub )
{
	...
	if( uSub == 0 )												// Haupteintrag bearbeiten
	{
		...
		if( uBits & 4 )											// Volle Spaltenbreite
		{
			...
			{
				sRect.right = pData->aColumnXpos[1];
				sRect.right -= pData->uScrollX;
			}
	...
	if( lRet&TVIR_EDITCOMBOBOX )								// Die Combobox füllen
	{
		...
		if( sNotify.pTextList )								// Texte über Listenfeld
		{
			for( uCnt = 0; uCnt < uMax; uCnt++ )
			{
				...
				if( !_tcscmp( pText, sNotify.item.pszText ) )
				{
					iSel = uCnt;
				}
			}
		...
		{												// Text mit
			...
			for( uCnt = 0; uCnt < uMax; uCnt++ )
			{
				...
				if( wParam >= 0x10000 && !_tcscmp( cText, sNotify.item.pszText ) )
				{
					iSel = uCnt;
				}
...
static int TreeListStartAutoEdit( TreeListData *pData, unsigned uColumn, WPARAM wParam, LPARAM lParam )
{
	...
	if( uMode >= TVAX_STEP )
	{
		...
		{													// Textliste  char *pText="1|2|3";
			...
			memset( cBuffer, 0, 256 * sizeof( TCHAR ) );
...
static LRESULT CALLBACK TreeListProc( HWND hWnd, UINT uMsg, WPARAM wParam, LPARAM lParam )
{
	...
	switch( uMsg )
	{
		case WM_CREATE:												// Das Fenster erzeugen
			...
			pData->fVAlternateMode = false;
			pData->uCheckedBkgColor = RGB( 255, 255, 255 );
			pData->uUnCheckedBkgColor = RGB( 255, 255, 255 );
		...
		case WM_DESTROY:											// Das Fenster zerstören
			...
			if( THEMEIMGLIST == pData->hStates )
				pData->hStates = 0;

			if( THEMEIMGLIST == pData->hChecks )
				pData->hChecks = 0;
		...
		case WM_SIZE:
			...
			if( uVal && uVal != pData->uSizeX )
			{
				...
				// Variable-width columns to perform...
			...
			if( uVal && uVal != pData->uSizeY )
			{
				...
				// If height is increased then check scroll position...
				if( uFlag && pData->uPageEnties > 2 )
				{
		...
		case WM_HSCROLL:											// Horizontal scrolling
			...
			if( iPos + (int)pData->uSizeX > pData->iMaxSizeX )
				iPos = pData->iMaxSizeX - pData->uSizeX;
			...
			CWnd::FromHandle( GetParent( hWnd ) )->SendMessage( WM_HSCROLL, wParam, (LPARAM)hWnd );
		...
		case WM_NOTIFY:												// Notify Nachrichnten
			...
			if( wParam == 1 )
			{
				...
				if( pHdr->hdr.code == HDN_BEGINTRACK )
				{
					pData = GetHandle( hWnd );
					...
					if( pData->uStyleEx&TVS_EX_FIXEDCOLSIZE )		// Fixe gesammte Spaltenbreite
					{
						for( iNext = pHdr->iItem + 1;; iNext++ )
						{
							...
							if( pData->aColumn[iNext].bFixed == 0 )
							{
								break;
							}
						}
					...
					if( U( pHdr->iItem ) < pData->uColumnCount )		// Darf die Spalte verändert werden
						if( pData->aColumn[U( pHdr->iItem )].bFixed )
						{
							return 1;
						}
				}
				...
				if( pHdr->hdr.code == HDN_TRACK || pHdr->hdr.code == HDN_ENDTRACK )
				{
					...
					if( pData->uStyleEx&TVS_EX_FIXEDCOLSIZE )		// Fixe gesammte Spaltenbreite 
					{
						...
						for( iNext = iCol + 1;; iNext++ )				// Überspringe fixierte Spalten
						{
							if( U( iNext ) >= pData->uColumnCount )
							{
								UNLOCK( pData );
						...
						sTemp.cxy = pData->aColumn[iNext].sReal + iDelta;
						...
						if( iDelta < 0 )							// Nächste Spalte wird zu klein
						{
							...
							{
								sTemp.cxy = 0;
								iDelta = -pData->aColumn[iNext].sReal;
								sItem.cxy = pData->aColumn[iCol].sReal - iDelta;
							}

							if( pData->aColumn[iNext].bMinEx )	// Minimale Breite prüfen
								if( pData->aColumn[iNext].sMin>sTemp.cxy )
								{
									iDelta += pData->aColumn[iNext].sMin - sTemp.cxy;
									sTemp.cxy = pData->aColumn[iNext].sMin;
									sItem.cxy = pData->aColumn[iCol].sReal - iDelta;
								}
						...
						if( pData->aColumn[iNext].bWeight )		// Variable Spalte
						{
							pData->iVarSize -= pData->aColumn[iNext].sSize;
							pData->iVarSize += sTemp.cxy;
						}
						else
						{									// Fixe Spalte
							pData->iFixSize -= pData->aColumn[iNext].sSize;
							pData->iFixSize += sTemp.cxy;
						}

						sTemp.mask = HDI_WIDTH;

						pData->aColumn[iNext].sSize = (short)sTemp.cxy;
						pData->aColumn[iNext].sReal = (short)sTemp.cxy;
						Header_SetItem( pData->hHeader, iNext, &sTemp );
					}
		...
		case TVM_SETIMAGELIST:										// Die Image-Liste einstellen
			...
			switch( (int)wParam )
			{
				...
				case TVSIL_CHECK:
					...
					else if( THEMEIMGLIST == pData->hChecks )
					{
						pData->iChecksXsize = 16;
						pData->iChecksYsize = 16;
						pData->iChecksMode = 1;
		...
		case TVM_SETITEMBKCOLOR:									// Hintergrundfarbe eines Eintrages ändern
			...
			LOCK( pData );
			lRet = TreeListSetItemColor( pData, U( wParam & 0xFFFFFF ), wParam >> 24, (COLORREF)lParam, 0 );
			UNLOCK( pData );
		...
		case TVM_SETITEMTEXTCOLOR:									// Textfarbe eines Eintrages ändern
			...
			LOCK( pData );
			lRet = TreeListSetItemColor( pData, U( wParam & 0xFFFFFF ), wParam >> 24, (COLORREF)lParam, 1 );
			UNLOCK( pData );
		...
		case TVM_SETCOLUMN:											// Einen Spalten-Header einstellen
			...
			if( pData->hHeader && U( wParam ) < pData->uColumnCount )
			{
				...
				{
					sItem.mask |= HDI_FORMAT; sItem.fmt = pCol->fmt;
				}
				...
				{
					sItem.mask |= HDI_WIDTH;  sItem.cxy = pCol->cx; pData->aColumn[U( wParam )].sSize = (short)sItem.cxy;
				}
				...
				if( lRet && ( pCol->mask&TVCF_FIXED ) )				// Die Spalte fixieren
				{
					pData->aColumn[U( wParam )].bFixed = (BYTE)( ( pCol->fmt&TVCFMT_FIXED ) ? 1 : 0 );
				}
				...
				if( lRet && ( pCol->mask&TVCF_MARK ) )				// Die Spalte markieren
				{
					...
					if( uVal != pData->aColumn[uPos].bMark )
					{
						pData->aColumn[uPos].bMark = (BYTE)uVal;
						pData->uMarkedCols += uVal * 2 - 1;

						sRect.left = pData->aColumnXpos[uPos];
						sRect.right = pData->aColumnXpos[uPos + 1] + 1;
		...
		case TVM_TAHYDRONICSMETHODS:
			pData = GetHandle( hWnd );
			switch( (int)wParam )
			{
				case TAMETHODCODE_GETTREELISTDATA:
					LOCK( pData );
					lRet = (LPARAM)pData;
					UNLOCK( pData );
					break;

				case TAMETHODCODE_ENSUREVISIBLE:
					lRet = TAEnsureVisible( pData, (TAEnsureVisibleParam*)lParam );
					break;

				case TAMETHODCODE_GETITEMRECT:
					lRet = TAGetItemRect( pData, (TAGetItemRectParam*)lParam );
					break;

				case TAMETHODCODE_SETVALTERNATECOLOR:
					pData->fTADrawAllLine = ( (TASetVAlternateColorParam*)lParam )->fDrawAllLine;
					pData->fVAlternateMode = ( (TASetVAlternateColorParam*)lParam )->fVAlternateMode;
					pData->uCheckedBkgColor = ( (TASetVAlternateColorParam*)lParam )->uCheckedBkgColor;
					pData->uUnCheckedBkgColor = ( (TASetVAlternateColorParam*)lParam )->uUnCheckedBkgColor;
					_TAComputeWiderItem( pData );
					lRet = 0;
					break;
			}
...
static void TreeListDraw( HWND hWnd, HDC hDc, RECT *pRect )
{
	...
	if( pData->uStyleEx & TVS_EX_ALTERNATECOLOR )					// Alternating colors
	{
	...
	if( !pData->cIsEnabled )										// If window style disabled gray
	{
		if( pData->uStyleEx & TVS_EX_GRAYEDDISABLE )
		{
			// uBkColor = pData->uColors[TVC_GRAYED];
			uBkColor = GetSysColor( COLOR_3DFACE );
			uEvColor = uEcColor;
			uOdColor = uOcColor;
		}
	}
	...
	// IMI Hydronic Engineering: keep a vector with all level and current state.
	// Remark: it's to know how we must paint background in the vertical alternate mode.
	std::vector<bool> vecLevelState;
	if( true == pData->fVAlternateMode && uPos > 0 )
	{
		for( unsigned uLoop = 0; uLoop < uPos; uLoop++ )
		{
			uItem = pData->pItemPos[uLoop];
			pEntry = pData->pTreeItems[uItem];
			if( NULL == pEntry )
				break;
			...
			bool fChecked = ( 1 == ( ( ( uBits & LVIS_STATEIMAGEMASK ) >> 12 ) & 0x0F ) ) ? true : false;
			if( ( pEntry->uLevel + 1 ) > vecLevelState.size() )
				vecLevelState.push_back( fChecked );
			else
				vecLevelState[pEntry->uLevel] = fChecked;
		}
	}
	...
	for( ; uPos < uMax; uPos++ )
	{
		...
		else if( uPos & 1 )											// Color change alternately
		{
		...
		if( pData->aColumn[0].bMark )								// Is the first column marked?
		{
			uBkColor = pData->uColors[TVC_COLBK];
		}

		// IMI Hydronic Engineering.
		if( 0 == pData->cIsEnabled )
			SetTextColor( hDc, GetSysColor( COLOR_GRAYTEXT ) );

		if( false == pData->fVAlternateMode || 0 == pData->cIsEnabled )
			SetBkColor( hDc, ( uStyleEx & TVS_EX_FULLROWITEMS ) ? uOutColor : uBkColor );
		else
			SetBkColor( hDc, ( false == vecLevelState[0] ) ? pData->uUnCheckedBkgColor : pData->uCheckedBkgColor );
		...
		// If 'TVS_HASLINES' style is active (Uses lines to show the hierarchy of items)...
		if( uStyle & TVS_HASLINES )
		{
			pTemp = pData->pTreeItems[pEntry->uParent];

			// Draw a row of pixels at the beginning.
			sArea.right = sArea.left + 1;
			ExtTextOut( hDc, 0, 0, ETO_OPAQUE, &sArea, NULL, 0, NULL );

			// New left coordinate is set to the current level multiply by number of pixel between each vertical line for level.
			...
			for( i = iLevel; i > 0; i-- )
			{
				...
				// IMI Hydronic Engineering.
				if( true == pData->fVAlternateMode && 1 == pData->cIsEnabled )
				{
					if( i > 1 )
						SetBkColor( hDc, ( false == vecLevelState[i - 2] ) ? pData->uUnCheckedBkgColor : pData->uCheckedBkgColor );
					else
						SetBkColor( hDc, ( false == vecLevelState[i - 1] ) ? pData->uUnCheckedBkgColor : pData->uCheckedBkgColor );
				}
				...
				// If previous item exist...
				if( pTemp )
				{
					// If there is a sibling on this previous item...
					if( pTemp->uNextItem )
					{
						// Draw vertical lines.
		...
		else
		{
			...
			{
				// Draw all vertical lines.
				if( false == pData->fVAlternateMode )
				{
				...
				else
				{
					...
					for( i = iLevel; i > 0; i-- )
					{
						sArea.right = sArea.left;
						sArea.left -= iIndent;
						SetBkColor( hDc, ( false == vecLevelState[i] ) ? pData->uUnCheckedBkgColor : pData->uCheckedBkgColor );
						ExtTextOut( hDc, 0, 0, ETO_OPAQUE, &sArea, NULL, 0, NULL );
					}
				}
			}
		...
		{
			if( pEntry->uLevel > 0 )
				SetBkColor( hDc, ( false == vecLevelState[pEntry->uLevel - 1] ) ? pData->uUnCheckedBkgColor : pData->uCheckedBkgColor );
			else
				SetBkColor( hDc, ( false == vecLevelState[pEntry->uLevel] ) ? pData->uUnCheckedBkgColor : pData->uCheckedBkgColor );
		}

		// If tree has expand/collapse buttons...
		if( uStyle & TVS_HASBUTTONS )
		{
			...
			if( pEntry->bFlags & TVIX_HASBUTTON )					// Feature buttons
			{
				...
				else
				{
					...
					// Draw horizontal line of symbol ( + or - contain both the horizontal line).
					...
					{
						// Draw vertical line if button is in collapsed mode (+).
						sButton.left = iXpos;
						sButton.top = iYpos - 2;
		...
		else if( uStyle & TVS_HASLINES )					// Only draw lines without buttons
		{
			sArea.right = sArea.left + iIndent;
			ExtTextOut( hDc, 0, 0, ETO_OPAQUE, &sArea, NULL, 0, NULL );
		...
		// If there is a border all around the item (included buttons, icons and text)...
		if( uStyleEx & TVS_EX_ITEMLINES )
		{
			iAdd = 1;
			sArea.right = sArea.left + 1;

			// If border is not limited to the item but goes to left origin...
			if( uStyleEx & TVS_EX_FULLROWITEMS )
			{
				// Thus, there is no vertical line before item.
				iStart = sArea.left;
				iAdd = 0;
			}
			else if( iLevel >= 0 )
		...
		if( pData->hStates )
		{
			...
			else if( pEntry->uStateEx & TVIS_EX_DISABLED )
			{
				// IMI Hydronic Engineering: add a disable state.
				if( pData->uStyleEx & TVS_EX_BITCHECKBOX )
					i++;

				if( i >= 1 && i <= 2 )
				{
					uState = ( i == 1 ) ? CBS_UNCHECKEDDISABLED : CBS_CHECKEDDISABLED;
					pDrawThemeBackg( pData->hThemeBt, hDc, BP_CHECKBOX, uState, &sArea, 0 );
				}
			}
			else
			{
				// IMI Hydronic Engineering: show grayed mode if tree is disabled.
				// 				if( 0 == pData->cIsEnabled )
				// 					i += 2;
		...
		// IMI Hydronic Engineering.
		if( true == pData->fVAlternateMode && false == pData->fTADrawAllLine && 1 == pData->cIsEnabled )
			SetBkColor( hDc, pData->uUnCheckedBkgColor );

		// If there is an icon before the current entry...
		if( pData->hImages && iImage != TV_NOIMAGE )
		{
			// If image comes from a sub-image list...
		...
		sArea.right = pData->aColumnXpos[1];					// Text output from the main entry
		iYpos = sArea.top + ( iHeight - pData->iFontHeight ) / 2;

		// IMI Hydronic Engineering: special color for a disabled item.
		COLORREF rPrevTextColor;
		if( pEntry->uStateEx & TVIS_EX_DISABLED && 1 == pData->cIsEnabled )
		{
			rPrevTextColor = GetTextColor( hDc );
			SetTextColor( hDc, GetSysColor( COLOR_GRAYTEXT ) );
		}

		// If text if formatted...
		// Remark: TVIS_SELECTED (Windows): The item is selected.
		//         TVIS_DROPHILITED (Windows): The item is selected as a drag-and-drop target.
		//		   TVIS_UNTERLINE (TreeListWnd): The item is underlined.
		//		   TVIS_TRACKED (TreeListWnd): The item is marked as drag-and-dropable (underline and blue)
		if( uBits & ( TVIS_SELECTED | TVIS_DROPHILITED | TVIS_UNTERLINE | TVIS_TRACKED | TVIS_TEXTCOLOR | TVIS_FOCUSED ) )
		{
			...
			// Does the text in the column
			// if(sButton.left+pEntry->iTextPixels>=(int)(sArea.right-pData->uScrollX))
			if( sButton.left - (int)pData->uScrollX + pEntry->iTextPixels >= (int)( sArea.right - (int)pData->uScrollX ) )
			{
			...
			else
			{
				...
				if( uBits & TVIS_FOCUSED )							// A dotted frame marks around the text
				{
					SelectObject( hDc, GetStockObject( NULL_BRUSH ) );
					Rectangle( hDc, sButton.left - 2, sButton.top - 1, sButton.right + 2, sButton.bottom + 1 );
				}
			...
			if( uBits & ( TVIS_UNTERLINE | TVIS_TRACKED ) )				// Apply an underline
			{
				if( pText && *pText )
				{
					sButton.left -= pData->iFontOff;
		...
		{													// The field normally spend
			if( !pEntry->iTextPixels && uTextSize )
			{
				...
				// Predictable text width
				DrawText( hDc, pText, uTextSize, &sButton, DT_LEFT | DT_VCENTER | DT_SINGLELINE | DT_NOPREFIX | DT_CALCRECT );

				pEntry->iTextPixels = sButton.right - sButton.left;
			}

			// If the text is larger than the column...
			//			if(sArea.left+pEntry->iTextPixels>=(int)(sArea.right-pData->uScrollX))
			if( sArea.left - (int)pData->uScrollX + pEntry->iTextPixels >= (int)( sArea.right - (int)pData->uScrollX ) )
			{
				...
				// Adjust or letters before "..."
				if( iCount > 0 )
				{
					// sButton.right	= sArea.right;
					// sArea.right		= sArea.left+2+pPos[iCount-1];
					sButton = sArea;
					sButton.right = sArea.left + 2 + pPos[iCount - 1];

					ExtTextOut( hDc, sButton.left + 2, iYpos, ETO_OPAQUE | ETO_CLIPPED, &sButton, pText, iCount, NULL );
					...
					ExtTextOut( hDc, sButton.left, iYpos, ETO_OPAQUE | ETO_CLIPPED, &sButton, _T("..."), 3, NULL );
				}
				else
				{
		...
		// IMI Hydronic Engineering: restore previous color.
		if( pEntry->uStateEx & TVIS_EX_DISABLED && 1 == pData->cIsEnabled )
		{
			SetTextColor( hDc, rPrevTextColor );
		}
				...
				{											// Text ausgeben in spezilem Format
					...
					else
					{
						...
						else if( uBits&TVIS_SELECTED && uItem == pData->uSelectedItem )
						{
							...
							if( pData->uStyleEx&TVS_EX_FULLROWMARK )
								SelectObject( hDc, GetStockObject( NULL_BRUSH ) );
							else	SelectObject( hDc, GetSysColorBrush( COLOR_3DFACE ) );
	...
	// If lower surface with no entries fill...
	if( sArea.top < sRect.bottom )
	{
		SelectObject( hDc, hRgnMain );

		// Are there marked columns...
	...
	// If we need to actualize horizontal scrollbar...
...
static int TAEnsureVisible( TreeListData* pData, TAEnsureVisibleParam* pParam )
{
	if( NULL == pParam )
		return -1;
	if( pParam->uItem > pData->uTreeItemsMax )
		return -1;

	BaseItem* pEntry = pData->pTreeItems[pParam->uItem];
	if( NULL == pEntry )
		return -1;

	CPoint ptCoord;
	// Remark: If item is before the left border of the client area, we have to know width of all the item. (we want to show all the item 
	//         included button, state, icon and text) -> pParam->fMoveLeft = true -> pass 'false' for 'fTextOnly'.
	//         If item is after the right border, we have to know the width of the text only (we just want to be able to show end of the text).
	_TAGetItemCoord( pData, pParam->uItem, ptCoord, !pParam->fMoveLeft );
	...
	GetScrollInfo( pData->hWnd, SB_HORZ, &rScrollInfo );

	// Retrieve parent size.
	CWnd* pParent = CWnd::FromHandle( GetParent( pData->hWnd ) );
	if( NULL == pParent )
		return -1;

	CRect rectClient;
	GetClientRect( pData->hWnd, &rectClient );

	if( true == pParam->fMoveLeft )
	{
		// Check if start of the item is before the left border of the client area.
		if( ptCoord.x < rScrollInfo.nPos )
		{
			int iNewPos = ptCoord.x;
			pData->uScrollX = iNewPos;
			SetScrollPos( pData->hWnd, SB_HORZ, iNewPos, TRUE );
			ScrollWindow( pData->hWnd, rScrollInfo.nPos - iNewPos, 0, NULL, NULL );

			pParent->SendMessage( WM_HSCROLL, MAKEWPARAM( -1, iNewPos - rScrollInfo.nPos ), (LPARAM)pData->hWnd );
		}
	}
	else
	{
		// Check if text of the item goes past the right border of the client area.
		if( ptCoord.y > rScrollInfo.nPos + rectClient.Width() )
		{
			int iNewPos = ptCoord.y - rectClient.Width();
			if( iNewPos + rectClient.Width() > rScrollInfo.nMax )
				iNewPos = rScrollInfo.nMax - rectClient.Width();
...
static int TAGetItemRect( TreeListData* pData, TAGetItemRectParam* pParam )
{
	...
	CPoint PtCoord;
	_TAGetItemCoord( pData, pParam->uItem, PtCoord, pParam->fTextOnly );

	pParam->Rect.top = 0;
	pParam->Rect.bottom = pData->iRowHeight;
	pParam->Rect.left = PtCoord.x;
	pParam->Rect.right = PtCoord.y;
...
static void _TAComputeWiderItem( TreeListData* pData )
{
	...
	// Remark: pData->uItemPosCount returns number of visible items. Children of an item collapsed are not count.
	pData->uTAWiderItem = 0;
	for( unsigned uLoop = 0; uLoop < pData->uItemPosCount; uLoop++ )
	{
		CPoint cPoint;
		unsigned uItem = pData->pItemPos[uLoop];
		BaseItem* pEntry = pData->pTreeItems[uItem];
		if( NULL == pEntry )
			return;

		_TAGetItemCoord( pData, uItem, cPoint, false );
		if( (unsigned)cPoint.y > pData->uTAWiderItem )
			pData->uTAWiderItem = cPoint.y;
	}
}
...
{
	BaseItem* pEntry = pData->pTreeItems[uItem];
	int iLeft = pData->iIndent * pEntry->uLevel;
	...
	// If tree has expand/collapse buttons...
	if( pData->uStyle & TVS_HASBUTTONS || pData->uStyle & TVS_HASLINES )
		iLeft += pData->iIndent;

	// If there is no border all around the item (included buttons, icons and text) or
	// if border is limited to the item...
	if( ( 0 == ( pData->uStyleEx & TVS_EX_ITEMLINES ) ) || ( 0 == ( pData->uStyleEx & TVS_EX_FULLROWITEMS ) ) )
		iLeft += 1;

	// If checkbox shown...
	if( pData->hStates )
		iLeft += pData->iStatesXsize;

	unsigned int uMark = 0xFFFFFFFF;
	if( ( pEntry->uState & TVIS_SELECTED ) && ( pData->uStyleEx & TVS_EX_FULLROWMARK ) )
		uMark = (unsigned)~TVIS_BKCOLOR;

	unsigned int uBits = pEntry->uState & 0xFFFF;
	uBits |= pEntry->bFlags << 16;
	uBits &= uMark;
	int iImage = ( uBits & LVIS_SELECTED ) ? pEntry->iSelectedImage : pEntry->iImage;
	...
	if( pData->hImages && iImage != TV_NOIMAGE )
	{
		// If image comes from a sub-image list...
		if( iImage >= 0x40000000 )
			iLeft += pData->iSubImgXsize;
		else
			iLeft += pData->iImagesXsize;
	}

	int iRight = iLeft;

	// If length of text has been already computed...
	if( pEntry->iTextPixels > 0 )
	{
		// Coordinate where to start to write down the text.
		iLeft += 4;
		iRight = iLeft + pEntry->iTextPixels;
	}
	else
	{
		// If special style...
		if( uBits & ( TVIS_SELECTED | TVIS_DROPHILITED | TVIS_UNTERLINE | TVIS_TRACKED | TVIS_TEXTCOLOR | TVIS_FOCUSED ) )
		{
			RECT sButton;
			sButton.top = 0;
			sButton.left = iLeft + 4;
			sButton.right = pData->aColumnXpos[1];
			sButton.bottom = pData->iFontHeight + 2;

			// If there is no text...
			if( 0 == pEntry->uTextSize )
			{
				iRight = iLeft + sButton.right - 2;
			}
			else
			{
				if( pData->uStyleEx & TVS_EX_FULLROWMARK )
				...
				DrawText( GetDC( pData->hWnd ), pEntry->pText, pEntry->uTextSize, &sButton, DT_LEFT | DT_VCENTER | DT_SINGLELINE | DT_NOPREFIX | DT_CALCRECT );
				iRight = iLeft + sButton.right - sButton.left;
			}
		}
		else
		{
			if( pEntry->uTextSize > 0 )
			{
				RECT sButton;
				sButton.top = 0;
				sButton.left = iLeft + 4;
