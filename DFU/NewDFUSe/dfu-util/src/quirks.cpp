...
void fixup_dfuse_layout(struct dfu_if *dif, struct memsegment **segment_list)
{
	if (dif->vendor == VENDOR_GIGADEVICE &&
		...
		if (!seg) {
			warnx(_T("Could not fix GD32VF103 layout because there ")
			      _T("is no segment at 0x%08x"), GD32VF103_FLASH_BASE);
			return;
		}
		...
		if (dif->serial_name[2] == 'B') {
			...
			warnx(_T("Unknown flash size '%c' in part number; ")
			      _T("defaulting to 128KB."), dif->serial_name[2]);
			count = 128;
		}
