...
const TCHAR* dfu_state_to_string( int state )
{
    const TCHAR *message;
    ...
    switch (state) {
        ...
        case STATE_APP_DETACH:
            message = _T("appDETACH");
            break;
        case STATE_DFU_IDLE:
            message = _T("dfuIDLE");
            break;
        case STATE_DFU_DOWNLOAD_SYNC:
            message = _T("dfuDNLOAD-SYNC");
            break;
        case STATE_DFU_DOWNLOAD_BUSY:
            message = _T("dfuDNBUSY");
            break;
        case STATE_DFU_DOWNLOAD_IDLE:
            message = _T("dfuDNLOAD-IDLE");
            break;
        case STATE_DFU_MANIFEST_SYNC:
            message = _T("dfuMANIFEST-SYNC");
            break;
        case STATE_DFU_MANIFEST:
            message = _T("dfuMANIFEST");
            break;
        case STATE_DFU_MANIFEST_WAIT_RESET:
            message = _T("dfuMANIFEST-WAIT-RESET");
            break;
        case STATE_DFU_UPLOAD_IDLE:
            message = _T("dfuUPLOAD-IDLE");
            break;
...
/* Chapter 6.1.2 */
static const TCHAR *dfu_status_names[] = {
	/* DFU_STATUS_OK */
		_T("No error condition is present"),
	/* DFU_STATUS_errTARGET */
        _T("File is not targeted for use by this device"),
	/* DFU_STATUS_errFILE */
        _T("File is for this device but fails some vendor-specific test"),
	/* DFU_STATUS_errWRITE */
        _T("Device is unable to write memory"),
	/* DFU_STATUS_errERASE */
        _T("Memory erase function failed"),
	/* DFU_STATUS_errCHECK_ERASED */
        _T("Memory erase check failed"),
	/* DFU_STATUS_errPROG */
        _T("Program memory function failed"),
	/* DFU_STATUS_errVERIFY */
        _T("Programmed memory failed verification"),
	/* DFU_STATUS_errADDRESS */
        _T("Cannot program memory due to received address that is out of range"),
	/* DFU_STATUS_errNOTDONE */
        _T("Received DFU_DNLOAD with wLength = 0, but device does not think that it has all data yet"),
	/* DFU_STATUS_errFIRMWARE */
        _T("Device's firmware is corrupt. It cannot return to run-time (non-DFU) operations"),
	/* DFU_STATUS_errVENDOR */
        _T("iString indicates a vendor specific error"),
	/* DFU_STATUS_errUSBR */
        _T("Device detected unexpected USB reset signalling"),
	/* DFU_STATUS_errPOR */
        _T("Device detected unexpected power on reset"),
	/* DFU_STATUS_errUNKNOWN */
        _T("Something went wrong, but the device does not know what it was"),
	/* DFU_STATUS_errSTALLEDPKT */
        _T("Device stalled an unexpected request")
};


const TCHAR *dfu_status_to_string(int status)
{
...
int dfu_abort_to_idle(struct dfu_if *dif)
{
    ...
    if( ret < 0 )
    {
		errx( EX_IOERR, _T("Error sending dfu abort request") );
	}
    ...
    if( ret < 0 )
    {
		errx( EX_IOERR, _T("Error during abort get_status") );
	}
	
    if( dst.bState != DFU_STATE_dfuIDLE )
    {
		errx( EX_IOERR, _T("Failed to enter idle state on abort") );
	}
