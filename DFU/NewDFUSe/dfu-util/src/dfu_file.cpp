...
void dfu_progress_bar(const TCHAR *desc, unsigned long long curr,
		unsigned long long max)
{
	static TCHAR buf[PROGRESS_BAR_WIDTH + 1];
	...
	_tprintf(_T("\r%s\t[%s] %3llu%% %12llu bytes"), desc, buf,
...
int dfu_malloc( size_t size, void **ppMemory )
{
	if( NULL == ppMemory )
	{
		errx(EX_SOFTWARE, _T("'ppMemory' can't be null.") );
	}

	*ppMemory = malloc(size);

	if( *ppMemory == NULL )
	{
		errx(EX_SOFTWARE, _T("Cannot allocate memory of size %d bytes"), (int)size);
	}
...
uint32_t dfu_file_write_crc(int f, uint32_t crc, const void *buf, int size)
{
	...
	if (_write(f, buf, size) != size)
		err(EX_IOERR, _T("Could not write %d bytes to file %d"), size, f);
...
int dfu_load_file(struct dfu_file *file, enum suffix_req check_suffix, enum prefix_req check_prefix, int verbose )
{
	...
	if( !_tcscmp( file->name, _T("-") ) )
	{
...
#endif
		ret = dfu_malloc( STDIN_CHUNK_SIZE, (void **)&file->firmware );

		if( ret != 0 )
		{
			err(EX_SOFTWARE, _T("Could not allocate firmware buffer"));
		}
		...
		while( read_bytes == STDIN_CHUNK_SIZE )
		{
			...
			if( !file->firmware )
			{
				errx( EX_SOFTWARE, _T("Could not allocate firmware buffer") );
			}
		...
		if( verbose )
		{
			_tprintf(_T("Read %lli bytes from stdin\n"), (long long) file->size.total);
		}
	...
	else
	{
		...
		f = _topen(file->name, O_RDONLY | O_BINARY);
		
		if (f < 0)
		{
			err( EX_NOINPUT, _T("Could not open file %s for reading"), file->name );
		}
		...
		if (offset < 0)
		{
			err(EX_SOFTWARE, _T("File size is too big"));
		}

		if (_lseek(f, 0, SEEK_SET) != 0)
		{
			err(EX_IOERR, _T("Could not seek to beginning"));
		}
		...
		{
			err(EX_SOFTWARE, _T("File too large for memory allocation on this platform"));
		}
		
		ret = dfu_malloc( file->size.total, (void **)&file->firmware );
		
		if( ret != 0 )
		{
			_close(f);
			free( file->firmware );
			errx( EX_SOFTWARE, _T("Could not allocate memory, returned %d"), ret);
		}
		...
		while (read_total < file->size.total)
		{
			...
			read_count = _read(f, file->firmware + read_total, to_read);
		...
		if (read_total != file->size.total)
		{
			_close(f);
			free( file->firmware );
			err( EX_IOERR, _T("Could only read %lld of %lld bytes from %s"), (long long) read_total, (long long) file->size.total, file->name);
		}
	...
	/* Check for possible DFU file suffix by trying to parse one */
	{
		...
		if (file->size.total < DFU_SUFFIX_LENGTH) {
			reason = _T("File too short for DFU suffix");
			missing_suffix = 1;
			goto checked;
		}
		...
		for (i = 0; i < file->size.total - 4; i++) crc = crc32_byte(crc, file->firmware[i]);

		if (dfusuffix[10] != 'D' || dfusuffix[9]  != 'F' || dfusuffix[8]  != 'U')
		{
			reason = _T("Invalid DFU suffix signature");
			missing_suffix = 1;
			goto checked;
		}
		...
		if( file->dwCRC != crc )
		{
			reason = _T("DFU suffix CRC does not match");
			missing_suffix = 1;
			goto checked;
		}
		...
		if( verbose )
		{
			_tprintf(_T("DFU suffix version %x\n"), file->bcdDFU);
		}
		...
		{
			errx( EX_DATAERR, _T("Unsupported DFU suffix length %d"), file->size.suffix );
		}

		if( file->size.suffix > file->size.total)
		{
			errx( EX_DATAERR, _T("Invalid DFU suffix length %d"), file->size.suffix );
		}
...
checked:
		if( missing_suffix )
		{
			...
			{
				warnx( _T("%s"), reason );
				errx( EX_DATAERR, _T("Valid DFU suffix needed") );
			}
			else if( check_suffix == MAYBE_SUFFIX )
			{
				warnx( _T("Warning: %s"), reason );
				warnx( _T("A valid DFU suffix will be required in a future dfu-util release") );
			}
		}
		else
		{
			...
			{
				errx( EX_DATAERR, _T("Please remove existing DFU suffix before adding a new one.\n") );
			}
		}
	}
	...
	{
		errx( EX_DATAERR, _T("Valid DFU prefix needed") );
	}

	if( file->size.prefix && check_prefix == NO_PREFIX )
	{
		errx( EX_DATAERR, _T("A prefix already exists, please delete it first") );
	}
	...
	if( file->size.prefix && verbose )
	{
		...
		{
			_tprintf(_T("Possible TI Stellaris DFU prefix with ")
					_T("the following properties\n")
					_T("Address:        0x%08x\n")
					_T("Payload length: %d\n"),
				   file->lmdfu_address,
				   data[4] | (data[5] << 8) |
		...
		{
			_tprintf(_T("Possible unencrypted NXP LPC DFU prefix with ")
					_T("the following properties\n")
					_T("Payload length: %d kiByte\n"),
				   data[2] >>1 | (data[3] << 7) );
		}
		else
		{
			errx( EX_DATAERR, _T("Unknown DFU prefix type") );
		}
	}
...
int dfu_store_file(struct dfu_file *file, int write_suffix, int write_prefix)
{
	...
	f = _topen(file->name, O_WRONLY | O_BINARY | O_TRUNC | O_CREAT, 0666);
	if (f < 0)
	{
		errx(EX_CANTCREAT, _T("Could not open file %s for writing"), file->name);
	}
...
void show_suffix_and_prefix(struct dfu_file *file)
{
	if (file->size.prefix == LMDFU_PREFIX_LENGTH) {
		_tprintf(_T("The file %s contains a TI Stellaris DFU prefix with the following properties:\n"), (LPCTSTR)file->name);
		_tprintf(_T("Address:\t0x%08x\n"), file->lmdfu_address);
	} else if (file->size.prefix == LPCDFU_PREFIX_LENGTH) {
		uint8_t * prefix = file->firmware;
		_tprintf(_T("The file %s contains a NXP unencrypted LPC DFU prefix with the following properties:\n"), (LPCTSTR)file->name);
		_tprintf(_T("Size:\t%5d kiB\n"), prefix[2]>>1|prefix[3]<<7);
	} else if (file->size.prefix != 0) {
		_tprintf(_T("The file %s contains an unknown prefix\n"), (LPCTSTR)file->name);
	}
	if (file->size.suffix > 0) {
		_tprintf(_T("The file %s contains a DFU suffix with the following properties:\n"), (LPCTSTR)file->name);
		_tprintf(_T("BCD device:\t0x%04X\n"), file->bcdDevice);
		_tprintf(_T("Product ID:\t0x%04X\n"),file->idProduct);
		_tprintf(_T("Vendor ID:\t0x%04X\n"), file->idVendor);
		_tprintf(_T("BCD DFU:\t0x%04X\n"), file->bcdDFU);
		_tprintf(_T("Length:\t\t%i\n"), file->size.suffix);
		_tprintf(_T("CRC:\t\t0x%08X\n"), file->dwCRC);
	}
}
