    ...
    int expected_size, int fd)
{
	...
	_tprintf(_T("Copying data from DFU device to PC\n"));

	while (1) {
		int rc;
		dfu_progress_bar(_T("Upload"), total_bytes, expected_size);
		rc = dfu_upload(dif->dev_handle, dif->interface, xfer_size, transaction++, (unsigned char *)buf);
		if (rc < 0) {
	...
	if (total_bytes == 0)
		...
		_tprintf(_T("Received a total of %lli bytes\n"), (long long) total_bytes);

	if (expected_size != 0 && total_bytes != expected_size)
		warnx(_T("Unexpected number of bytes uploaded from device"));
	return ret;
}
...
int dfuload_do_dnload(struct dfu_if *dif, int xfer_size, struct dfu_file *file, int verbose)
{
	...
	_tprintf(_T("Copying data from PC to DFU device\n"));
	...
	while (bytes_sent < expected_size) {
		...
		if (ret < 0) {
			warnx(_T("Error during download (%s)"),
		...
		do {
			...
			if (ret < 0) {
				errx(EX_IOERR, _T("Error during download get_status (%s)"),
				     libusb_error_name(ret));
				goto out;
			}
		...
		if (dst.bStatus != DFU_STATUS_OK) {
			_tprintf(_T(" failed!\n"));
			_tprintf(_T("DFU state(%u) = %s, status(%u) = %s\n"), dst.bState,
				dfu_state_to_string(dst.bState), dst.bStatus,
				dfu_status_to_string(dst.bStatus));
			ret = -1;
			goto out;
		}
		dfu_progress_bar(_T("Download"), bytes_sent, bytes_sent + bytes_left);
	}

	/* send one zero sized download request to signalize end */
	ret = dfu_download(dif->dev_handle, dif->interface, 0, transaction, NULL);
	if (ret < 0) {
		errx(EX_IOERR, _T("Error sending completion packet (%s)"),
		     libusb_error_name(ret));
		goto out;
	}

	dfu_progress_bar(_T("Download"), bytes_sent, bytes_sent);

	if (verbose)
		_tprintf(_T("Sent a total of %lli bytes\n"), (long long) bytes_sent);

get_status:
	/* Transition to MANIFEST_SYNC state */
	ret = dfu_get_status(dif, &dst);
	if (ret < 0) {
		warnx(_T("unable to read DFU status after completion (%s)"),
		      libusb_error_name(ret));
		goto out;
	}
	_tprintf(_T("DFU state(%u) = %s, status(%u) = %s\n"), dst.bState,
	...
	case DFU_STATE_dfuMANIFEST_WAIT_RST:
		_tprintf(_T("Resetting USB to switch back to runtime mode\n"));
		ret = libusb_reset_device(dif->dev_handle);
		if (ret < 0 && ret != LIBUSB_ERROR_NOT_FOUND) {
			_ftprintf(stderr, _T("error resetting after download (%s)\n"),
				libusb_error_name(ret));
		}
		break;
