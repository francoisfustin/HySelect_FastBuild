...
int add_segment(struct memsegment **segment_list, struct memsegment segment)
{
	...
	ret = dfu_malloc(sizeof(struct memsegment), (void **)&new_element );
 ...
 */
struct memsegment *parse_memory_layout( char *intf_desc, int verbose )
{
	...
	ret = dfu_malloc(strlen(intf_desc), (void **)&name );
	...
	if (ret < 1) {
		free(name);
		warnx(_T("Could not read name, sscanf returned %d"), ret);
		return NULL;
	}
	...
	ret = dfu_malloc(strlen(intf_desc), (void **)&typestring );

	if( ret != 0 )
	{
		warnx(_T("Could not alloc memory, returned %d"), ret);
		return NULL;
	}
		...
		while (ret = sscanf(intf_desc, "%d*%d%c%[^,/]%n",
			...
			if (ret == 4) {
				if (strlen(typestring) == 1
					...
					warnxA("Parsing type identifier '%s' "
			...
			case 'g':
				if (!memtype) {
					warnx(_T("Non-valid multiplier '%c', ")
						_T("interpreted as type ")
						_T("identifier instead"),
						multiplier);
					memtype = multiplier;
					break;
				}
			...
			default:
				warnx(_T("Non-valid multiplier '%c', ")
					_T("assuming bytes"), multiplier);
			}

			if (!memtype) {
				warnx(_T("No valid type for segment %d\n"), count);
				continue;
			}
			...
			ret = add_segment(&segment_list, segment);
			...
			if( ret != 0 )
			{
				...
				warnx(_T("Could not add segment, returned %d"), ret);
				return NULL;
			}
