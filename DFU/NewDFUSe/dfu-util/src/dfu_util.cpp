    ...
    uint8_t desc_type, void *res_buf, int res_size)
{
	...
	while (p + 1 < list_len) {
		...
		if (desclen == 0) {
			warnx(_T("Invalid descriptor list"));
			return -1;
		}
...
int get_utf8_string_descriptor( libusb_device_handle *devh, uint8_t desc_index, unsigned char *data, int length)
{
	...
	if (r < 0) {
		warnx(_T("Failed to retrieve language identifiers"));
		return r;
	}
	if (r < 4 || tbuf[0] < 4 || tbuf[1] != LIBUSB_DT_STRING) {		/* must have at least one ID */
		warnx(_T("Broken LANGID string descriptor"));
		return -1;
	}
	...
	r = libusb_get_string_descriptor(devh, desc_index, langid, tbuf,
					 sizeof(tbuf));
	if (r < 0) {
		warnx(_T("Failed to retrieve string descriptor %d"), desc_index);
		return r;
	}
	if (r < 2 || tbuf[0] < 2) {
		warnx(_T("String descriptor %d too short"), desc_index);
		return -1;
	}
	if (tbuf[1] != LIBUSB_DT_STRING) {	/* sanity check */
		warnx(_T("Malformed string descriptor %d, type = 0x%02x"), desc_index, tbuf[1]);
		return -1;
	}
	if (tbuf[0] > r) {	/* if short read,           */
		warnx(_T("Patching string descriptor %d length (was %d, received %d)"), desc_index, tbuf[0], r);
...
static int probe_configuration( libusb_device *dev, struct libusb_device_descriptor *desc, _DFUUtilWorkVariables *pDFUUtilWorkVariables )
{
	...
	for( cfg_idx = 0; cfg_idx != desc->bNumConfigurations; cfg_idx++ )
	{
		...
		if( pDFUUtilWorkVariables->match_config_index > -1 && pDFUUtilWorkVariables->match_config_index != cfg->bConfigurationValue)
		{
		...
		if( has_dfu > 0 )
		{
			 ...
			 */
			if( 0 == libusb_open( dev, &devh ) )
			{
				ret = libusb_get_descriptor( devh, USB_DT_DFU, 0, (unsigned char *)&func_dfu, sizeof( func_dfu ) );
				libusb_close( devh );
			...
			warnx( _T("Device has DFU interface, but has no DFU functional descriptor") );
...
found_dfu:
		if( 7 == func_dfu.bLength )
		{
			_tprintf( _T("Deducing device DFU version from functional descriptor length\n") );
			func_dfu.bcdDFUVersion = libusb_cpu_to_le16( 0x0100 );
		}
		else if( func_dfu.bLength < 9)
		{
			_tprintf( _T("Error obtaining DFU functional descriptor\n") );
			_tprintf( _T("Please report this as a bug!\n") );
			_tprintf( _T("Warning: Assuming DFU version 1.0\n") );
			func_dfu.bcdDFUVersion = libusb_cpu_to_le16( 0x0100 );
			_tprintf( _T("Warning: Transfer size can not be detected\n") );
		...
		for( intf_idx = 0; intf_idx < cfg->bNumInterfaces; intf_idx++)
		{
			...
			if( pDFUUtilWorkVariables->match_iface_index > -1 && pDFUUtilWorkVariables->match_iface_index != intf_idx )
			{
				continue;
			}
			...
			for( alt_idx = 0; alt_idx < uif->num_altsetting; alt_idx++ )
			{
				...
				if( dfu_mode && pDFUUtilWorkVariables->match_iface_alt_index > -1 && pDFUUtilWorkVariables->match_iface_alt_index != intf->bAlternateSetting )
				{
					continue;
				}
				...
				if( dfu_mode )
				{
					if( ( pDFUUtilWorkVariables->match_vendor_dfu >= 0 && pDFUUtilWorkVariables->match_vendor_dfu != desc->idVendor)
							|| ( pDFUUtilWorkVariables->match_product_dfu >= 0 && pDFUUtilWorkVariables->match_product_dfu != desc->idProduct ) )
					{
						continue;
					}
				}
				...
				{
					if( ( pDFUUtilWorkVariables->match_vendor >= 0 && pDFUUtilWorkVariables->match_vendor != desc->idVendor )
							|| ( pDFUUtilWorkVariables->match_product >= 0 && pDFUUtilWorkVariables->match_product != desc->idProduct ) )
					{
						continue;
					}
				}

				if( pDFUUtilWorkVariables->match_devnum >= 0 && pDFUUtilWorkVariables->match_devnum != libusb_get_device_address( dev ) )
				{
					continue;
				}
				...
				if( ret )
				{
					warnx(_T("Cannot open DFU device %04x:%04x found on devnum %i (%s)"),
				...
				{
					ret = get_string_descriptor_ascii( devh, intf->iInterface, (unsigned char *)alt_name, MAX_DESC_STR_LEN );
				}
				else
				{
					ret = -1;
				...
				if( desc->iSerialNumber != 0 )
				{
					...
					{
						ret = get_utf8_string_descriptor( devh, desc->iSerialNumber, (unsigned char *)serial_name, MAX_DESC_STR_LEN - 1 );
					...
					{
						ret = get_string_descriptor_ascii( devh, desc->iSerialNumber, (unsigned char *)serial_name, MAX_DESC_STR_LEN );
					}
				}
				else
				{
				...
				if( dfu_mode && pDFUUtilWorkVariables->match_iface_alt_name != NULL && strcmp( alt_name, pDFUUtilWorkVariables->match_iface_alt_name ) )
				{
					continue;
				}
				...
				if( dfu_mode )
				{
					if( pDFUUtilWorkVariables->match_serial_dfu != NULL && strcmp( pDFUUtilWorkVariables->match_serial_dfu, serial_name ) )
					{
						continue;
					}
				}
				...
				{
					if( pDFUUtilWorkVariables->match_serial != NULL && strcmp( pDFUUtilWorkVariables->match_serial, serial_name ) )
					{
						continue;
					}
				}

				ret = dfu_malloc( sizeof( *pdfu ), (void **)&pdfu );
				...
				{
					errx(EX_SOFTWARE, _T("Out of memory"));
				}
				...
				/* queue into list */
				pdfu->next = pDFUUtilWorkVariables->dfu_root;
				pDFUUtilWorkVariables->dfu_root = pdfu;
			}
		}
...
void probe_devices( libusb_context *ctx, _DFUUtilWorkVariables *pDFUUtilWorkVariables )
{
	if( NULL == pDFUUtilWorkVariables )
	{
		assert( 0 );
	...
	for( i = 0; i < num_devs; ++i )
	{
		...
		if( pDFUUtilWorkVariables->match_path != NULL && strcmp( get_path( dev ), pDFUUtilWorkVariables->match_path ) != 0 )
		{
			continue;
		}
		...
		probe_configuration( dev, &desc, pDFUUtilWorkVariables );
	}
...
void disconnect_devices( _DFUUtilWorkVariables *pDFUUtilWorkVariables )
{
	...
	for( pdfu = pDFUUtilWorkVariables->dfu_root; pdfu != NULL; pdfu = pdfu->next )
	{
		free( prev );
	...
	free( prev );
	pDFUUtilWorkVariables->dfu_root = NULL;
}

void print_dfu_if( struct dfu_if *dfu_if )
{
	printf( "Found %s: [%04x:%04x] ver=%04x, devnum=%u, cfg=%u, intf=%u, path=\"%s\", alt=%u, name=\"%s\", serial=\"%s\"\n",
