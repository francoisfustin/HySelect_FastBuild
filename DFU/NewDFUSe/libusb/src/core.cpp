...
{
	struct discovered_devs *ret = (struct discovered_devs *)malloc(sizeof(*ret) + (sizeof(void *) * DISCOVERED_DEVICES_SIZE_STEP));
	...
	struct discovered_devs *discdevs, struct libusb_device *dev)
{
	...
	new_discdevs = (struct discovered_devs *)realloc(discdevs, sizeof(*discdevs) + (sizeof(void *) * capacity));
...
int API_EXPORTED libusb_init(libusb_context **ctx)
{
	...
	if (NULL == ctx && default_context_options[LIBUSB_OPTION_LOG_LEVEL].is_set) {
		_ctx->debug = (libusb_log_level)default_context_options[LIBUSB_OPTION_LOG_LEVEL].arg.ival;
	} else {
	...
	for (int option = 0 ; option < LIBUSB_OPTION_MAX ; option++) {
		...
		r = libusb_set_option(_ctx, (libusb_option)option);
		if (LIBUSB_SUCCESS != r)
			goto err_free_ctx;
	}
 ...
 */
DEFAULT_VISIBILITY const TCHAR * LIBUSB_CALL libusb_error_name(int error_code)
{
	switch (error_code) {
	case LIBUSB_ERROR_IO:
		return _T("LIBUSB_ERROR_IO");
	case LIBUSB_ERROR_INVALID_PARAM:
		return _T("LIBUSB_ERROR_INVALID_PARAM");
	case LIBUSB_ERROR_ACCESS:
		return _T("LIBUSB_ERROR_ACCESS");
	case LIBUSB_ERROR_NO_DEVICE:
		return _T("LIBUSB_ERROR_NO_DEVICE");
	case LIBUSB_ERROR_NOT_FOUND:
		return _T("LIBUSB_ERROR_NOT_FOUND");
	case LIBUSB_ERROR_BUSY:
		return _T("LIBUSB_ERROR_BUSY");
	case LIBUSB_ERROR_TIMEOUT:
		return _T("LIBUSB_ERROR_TIMEOUT");
	case LIBUSB_ERROR_OVERFLOW:
		return _T("LIBUSB_ERROR_OVERFLOW");
	case LIBUSB_ERROR_PIPE:
		return _T("LIBUSB_ERROR_PIPE");
	case LIBUSB_ERROR_INTERRUPTED:
		return _T("LIBUSB_ERROR_INTERRUPTED");
	case LIBUSB_ERROR_NO_MEM:
		return _T("LIBUSB_ERROR_NO_MEM");
	case LIBUSB_ERROR_NOT_SUPPORTED:
		return _T("LIBUSB_ERROR_NOT_SUPPORTED");
	case LIBUSB_ERROR_OTHER:
		return _T("LIBUSB_ERROR_OTHER");

	case LIBUSB_TRANSFER_ERROR:
		return _T("LIBUSB_TRANSFER_ERROR");
	case LIBUSB_TRANSFER_TIMED_OUT:
		return _T("LIBUSB_TRANSFER_TIMED_OUT");
	case LIBUSB_TRANSFER_CANCELLED:
		return _T("LIBUSB_TRANSFER_CANCELLED");
	case LIBUSB_TRANSFER_STALL:
		return _T("LIBUSB_TRANSFER_STALL");
	case LIBUSB_TRANSFER_NO_DEVICE:
		return _T("LIBUSB_TRANSFER_NO_DEVICE");
	case LIBUSB_TRANSFER_OVERFLOW:
		return _T("LIBUSB_TRANSFER_OVERFLOW");
	case 0:
		return _T("LIBUSB_SUCCESS / LIBUSB_TRANSFER_COMPLETED");
	default:
		return _T("**UNKNOWN**");
	}
}
