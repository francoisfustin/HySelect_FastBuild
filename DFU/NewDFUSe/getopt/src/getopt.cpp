...
// This must be initialized on some systems to avoid linking in the system's own getopt implementation.
...
// gcc with -traditional declares the built-in strlen to return int, and has done so at least since version 2.4.5. -- rms.
...
static void exchange(char **argv )
{
  ...
  if (nonoption_flags_len > 0 && top >= nonoption_flags_max_len)
    {
      // We must extend the array.  The user plays games with us and presents new arguments.
...
# define NONOPTION_P (argv[optind][0] != '-' || argv[optind][1] == '\0')
...

  {
	...
	if (*nextchar != '\0')
	  ...
	  // increment it again when taking next ARGV-elt as argument.s
	  optarg = argv[optind++];
	...
	for (p = longopts, option_index = 0; p != NULL && p->name; p++, option_index++)
	  if (!strncmp (p->name, nextchar, nameend - nextchar))
		...
		// Second or later nonexact match found.s
		ambig = 1;
	    }
	if (ambig && !exact)
	  {
