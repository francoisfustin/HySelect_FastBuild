...
CImage::CImage(CImage *pSource)
{
	...
	if (m_bNamed)
		lstrcpyA(m_Name, pSource->m_Name);
			...
			else if (header[0] == 'S') 
			{
				...
				if (strcmp(header,"S0")==0) {
					...
					if (((checksum + sum_var +1) % 256) != 0) {
						free(Element.Data);
						wsprintfA(message, "FILE : line %i: Checksum error!",lineno);
					...
					if (byte_count == 3) {         // S5 must have 3 bytes (2 data & chk)
						...
						wsprintfA(message, "FILE : line %i: S5 line 'byte count' error!",lineno);
						LDisplayError(message);
						bRet=FALSE;
						break;
					}
					...
					if (byte_count == 5) {
						...
						wsprintfA(message, "FILE : line %i: S7 line 'byte count' error!",lineno);
						LDisplayError(message);
						bRet=FALSE;
						break;
					}
					...
					if (byte_count == 4) {
						...
						wsprintfA(message, "FILE : line %i: S8 line 'byte count' error!",lineno);
						LDisplayError(message);
						bRet=FALSE;
						break;
					}
					...
					if (byte_count == 3) { // The right format is this one S903xxxxChk
						...
						wsprintfA(message, "FILE : line %i: S9 line 'byte count' error!",lineno);
						LDisplayError(message);
						bRet=FALSE;
						break;
					}
					...
					wsprintfA(message, "FILE : line %i: Not in Motorola S19 format!", lineno);
					LDisplayError(message);
					bRet=FALSE;
					break;
				}
			...
			if (feof(fp))  { // Detect End Of File & Exit if S9 or S8 or S7 record not found
				...
				if (separator==0x00)
				{
					...
					if (((checksum + sum_var) % 256) != 0) {
						free(Element.Data);
						wsprintfA(message, "FILE : line %i: Bad hexadecimal checksum!", lineno);
						LDisplayError(message);
						bRet=FALSE;
						break;
					}
				...
				{
					wsprintfA(message, "FILE : line %i: Not in Intel Hex format!", lineno);
					LDisplayError(message);
					bRet=FALSE;
					break;
				}
		...
		for (i=0;i<GetNbElements();i++)
		{
			...
			while (first <= last) {
				...
				if (MaxAreaAddress > (unsigned long)0xFFFFFF) {
					wsprintfA(record, "S3%02X%08X", (unsigned short)length+5, (unsigned long)first);
