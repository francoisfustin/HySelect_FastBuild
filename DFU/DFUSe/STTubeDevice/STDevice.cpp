...
CSTDevice::CSTDevice(CStringA SymbolicName)
{
...
DWORD  CSTDevice::OpenDevice(PHANDLE phUnPlugEvent)
{
	...
	m_DeviceHandle=CreateFileA(	m_SymbolicName,
	...
	for(_j=0; _j<_uiNbOfInterfaces; _j++)
	{
		...
		for (i=0;i<(int)_uiNbOfEndpointsTmp;i++)
		{
			...
			if (nRet==STDEVICE_NOERROR)
			{
				...
				m_pPipeHandles[_k]= CreateFileA( PipeSymbName,
...
DWORD CSTDevice::GetStringDescriptor(UINT Index, CStringA &Desc)
{
	BOOL Success;
	DWORD ByteCount;
	BYTE	strID[4];
