/***********************************************************************
* SSOleDrg.cpp -- OLE Drag and drop support for Spread OCX controls
*
...
*/
#ifdef SS_V40
  if (m_bEventEnabled[EventEnabled_OLEGiveFeedback])
#endif // SS_V40
...
#ifdef SS_V40
  if (m_bEventEnabled[EventEnabled_OLEDragOver])
#endif // SS_V40
...
#ifdef SS_V40
  if (m_bEventEnabled[EventEnabled_OLEDragDrop])
#endif // SS_V40
...
#ifdef SS_V40
  if (m_bEventEnabled[EventEnabled_OLESetData])
#endif // SS_V40
...
#ifdef SS_V40
  if (m_bEventEnabled[EventEnabled_OLEStartDrag])
#endif // SS_V40
...
//		m_bDragging = FALSE;
		pDropSource->Release();
#ifdef SS_V40
  if (m_bEventEnabled[EventEnabled_OLECompleteDrag])
#endif // SS_V40
...
#ifdef SS_V40
  if (pThis->m_bEventEnabled[EventEnabled_OLEGiveFeedback])
#endif // SS_V40
...
STDMETHODIMP FPMYDISP_CTRL_NAME::XfpDropTarget::Drop(LPDATAOBJECT pDataObject, ULONG grfKeyState, POINTL pt, ULONG * pdwEffect)
{
	...
	//{
        ...
        LPSS_BOOK lpBook = pThis->LockBook();
#ifdef SS_V40
  if (pThis->m_bEventEnabled[EventEnabled_OLEDragDrop])
#endif // SS_V40
        if (lpBook->bScriptEnhanced)
           {
           VARIANT arg;
...
#ifdef SS_V40
  if (pThis->m_bEventEnabled[EventEnabled_OLESetData])
#endif // SS_V40
...
//           FireScriptOLEStartDrag(&m_pfpDataObject, &varParam);
	...
	{
		ULONG ResultEffect = 0L;
      int iSwapButton = GetSystemMetrics(SM_SWAPBUTTON);
      ...
      */
		if (GetAsyncKeyState(iSwapButton ? VK_RBUTTON : VK_LBUTTON) & 0x8000)
			m_xfpDropSource.m_nDragButton |= VK_LBUTTON;
		if(GetAsyncKeyState(iSwapButton ? VK_LBUTTON : VK_RBUTTON) & 0x8000)
			m_xfpDropSource.m_nDragButton |= VK_RBUTTON;
		if(GetAsyncKeyState(VK_MBUTTON) & 0x8000)
