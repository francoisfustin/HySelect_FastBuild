 ...
 *   They should exit to error_exit_jpeg if unsuccessful.
 *
...
/* Allocate a "small" object */
{
  ...
  if (hdr_ptr == NULL) {
    ...
    for (;;) {
//      hdr_ptr = (small_pool_ptr) fpGlobalAlloc (GHND, min_request + slop);
  ...
  data_ptr = (TCHAR *) (hdr_ptr + 1); /* point to first data byte in pool */
...
static jvirt_sarray_ptr request_virt_sarray (j_common_ptr cinfo, int pool_id, BOOL pre_zero,
...
static jvirt_barray_ptr request_virt_barray (j_common_ptr cinfo, int pool_id, BOOL pre_zero,
...
static void do_sarray_io (j_common_ptr cinfo, jvirt_sarray_ptr ptr, BOOL writing)
...
static void do_barray_io (j_common_ptr cinfo, jvirt_barray_ptr ptr, BOOL writing)
...
void jinit_memory_mgr (j_common_ptr cinfo)
{
  ...
  /* Attempt to allocate memory manager's control block */			// (my_mem_ptr) malloc (sizeof (my_memory_mgr));
  mem = (my_mem_ptr) malloc (SIZEOF(my_memory_mgr));

  if (mem == NULL) { // jb mem was null
	error_exit_jpeg (cinfo);
  }
